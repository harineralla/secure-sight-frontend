...
const RuntmeSensor = () => {
  ...
  const lessTime = new Date(dateNow.getTime() - time)
    .toISOString()
    .slice(0, -1);
  const getRuntmeSensorData = async () => {
    setOpenLoader(true);
    ...
    const response = await ApiServices(
      "post",
      time > 0 ? payload : "",
      ApiEndPoints.ContainerRuntimeSensor
    );
    setData(response);
    setOpenLoader(false);
          ...
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <Breadcrumbsub
                    title="Container Security Runtime Sensor"
                    breadcrumbItem={
